...
{
    public class agendaType3: agenda
    {
        private int _MaxSize;
        private int _count;
        private processSym[] _tab;
        private double przesuniecieCzasu;         // W celu przyspieszenia działania eksperymentu symulacyjnego, 


        public agendaType3(aSimulationType3 alg)
        {
            this.AlgSym = alg;
            _count = 0;
            _MaxSize = 1024;
        ...
        {
            dodawany.czas += przesuniecieCzasu;
            dodawany.idx = _count;
            incCount();
        ...
        public override processSym PobierzPierwszy()
        {
            ...
            naprawKopiec(0);
            wynik.czas -= przesuniecieCzasu;

            return wynik;
        }

        public void CofnijCzasy(double cofnocO)
        {
            ...
            if (przesuniecieCzasu > 1000)
            {
                for (int idx = 0; idx < _count; idx++)
                    _tab[idx].czas -= przesuniecieCzasu;
                przesuniecieCzasu = 0;
            }
        }
    ...
    /// </summary>
    public class aSimulationType3 : aSimulation
    {
        ...
        public aSimulationType3(Wiazka wSymulowana)
            : base(wSymulowana, 3, wSymulowana.bazaDanych, "bezwzględny czas, statystyki na zmiennych typu double, dodawane bez optymalizacji")
        {
            SkrNazwaAlg = "Sym3";
            NazwaAlg = "Symulacja typ 3";
            _symulacja = true;
        }
